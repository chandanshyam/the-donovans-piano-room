...
const blackKeys = [
  ...
  const ShortPiano: React.FC<{ volume: number; showNotes: boolean }> = ({ volume, showNotes }) => {
      ...
      <div>
        <div className="relative m-20 flex h-5/6 pt-10 ">
          ...
          <div className="lg:w-18 2xl:w-22 ml-[-496%] flex w-8 sm:w-10 md:w-12 xl:w-20  2xl:h-3/5">
            ...
            <div className={`absolute z-30 ml-[-6%] mt-[300%] flex w-full ${showNotes ? 'block' : 'hidden'}`}>
              {whiteKeys.map((note, index) => (
                <span
                  key={index}
                  onClick={() => playNote(note)}
          ...
          <div className={`-ml-[32%] text-xs sm:text-sm md:text-lg lg:text-lg xl:text-2xl ${showNotes ? 'block' : 'hidden'}`}>
            {blackKeys.map((item, index) => (
              <span
                key={index}
                onClick={() => playNote(item.note)}
